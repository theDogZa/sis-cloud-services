...
<div class="container-fluid">
    ...
    <h2 class="content-heading pt-2">
        ...
        <div class="bock-sub-menu">
            @permissions('create.roles_permissions')
            @include('components.button._add')
            @endpermissions
        </div>
        @if(View::exists('_roles_permissions._advanced_search'))
        @include('_roles_permissions._advanced_search')
        @endif
    </h2>
    <!-- END Content Heading -->
    ...
    <div class="block {{config('theme.layout.main_block')}}">
        <div class="block-header {{config('theme.layout.main_block_header')}}">
            <h3 class="block-title">
                ...
                {{ ucfirst(__('roles_permissions.head_title.list')) }}
            ...
            <table class="table {{config('theme.layout.table_list_item')}}">
                <thead class="{{config('theme.layout.table_list_item_head')}}">
                    <tr>
                        ...
                        <th class="text-center">
                            @sortablelink('role_id',__('roles_permissions.role_id.th'))
                        </th>
                        @endif
                        @if($arrShowField['permission_id']==true)
                        <th class="text-center">
                            @sortablelink('permission_id',__('roles_permissions.permission_id.th'))
                        </th>
                        @endif
                        @if($arrShowField['active']==true)
                        <th class="text-center">
                            @sortablelink('active',__('roles_permissions.active.th'))
                        </th>
                        @endif
                ...
                <tbody>
                    ...
                    <tr id="row_{{$item->id}}">
                        ...
                        <td>
                            @include('components._badge_radio',['val'=>@$item->active,'tTrue'=>ucfirst(__('roles_permissions.active.text_radio.true')), 'tFalse'=>ucfirst(__('roles_permissions.active.text_radio.false'))])
                        </td>
                        @endif
                        <td class="text-center">
                            <div class="btn-group">
                                @permissions('read.roles_permissions')
                                @include('components.button._view')
                                @endpermissions
                                @permissions('update.roles_permissions')
                                @include('components.button._edit')
                                @endpermissions
                                @permissions('del.roles_permissions')
...
<!-- END Page Content -->
@include('components._form_del',['action'=> 'roles_permissions'])
