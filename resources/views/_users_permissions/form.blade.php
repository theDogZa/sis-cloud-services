    ...
    <div class="block {{config('theme.layout.main_block')}}">
        <div class="block-header {{config('theme.layout.main_block_header')}}">
            <h3 class="block-title">
                ...
                {{ ucfirst(__('users_permissions.head_title.add')) }}
                @else
                {{ ucfirst(__('users_permissions.head_title.edit')) }}
                @endif
                <small> </small>
            </h3>
        </div>
        ...
        <div class="block-content">
            <!-- ** Content Data ** -->
            <form action="{{ url('/users_permissions'.( isset($users_permission) ? '/' . $users_permission->id : '')) }}" method="POST" class="needs-validation" enctype="application/x-www-form-urlencoded" id="form" novalidate>
                {{ csrf_field() }}
                @if(isset($users_permission))
                ...
                <div class="row form-group">
                    ...
                    <div class="{{config('theme.layout.form')}}">
                        ...
                        <select class="form-control" id="user_id" name="user_id" required >
                         ...
                         @include('components._invalid_feedback',['required'=>'required','message'=>ucfirst(__('users_permissions.user_id.label')) ])
                    </div>
                    @endif
                    ...
                    <div class="{{config('theme.layout.form')}}">
                        ...
                        <select class="form-control" id="permission_id" name="permission_id" required >
                         ...
                         @include('components._invalid_feedback',['required'=>'required','message'=>ucfirst(__('users_permissions.permission_id.label')) ])
                    </div>
                    @endif
                    ...
                    <div class="{{config('theme.layout.form')}}">
                        ...
                        <input type="number" class="form-control" id="role_id" name="role_id"   value="{{@$role_id}}" placeholde="{{__('users_permissions.role_id.placeholder')}}">
                        @include('components._invalid_feedback',['required'=>'','message'=>ucfirst(__('users_permissions.role_id.label')) ])
                    </div>
                    @endif
