...
<div class="container-fluid">
    ...
    <h2 class="content-heading pt-2">
        ...
        @if(View::exists('_permissions._advanced_search'))
        @include('_permissions._advanced_search')
        @endif
    </h2>
    <!-- END Content Heading -->
    ...
    <div class="block {{config('theme.layout.main_block')}}">
        <div class="block-header {{config('theme.layout.main_block_header')}}">
            <h3 class="block-title">
                ...
                {{ ucfirst(__('permissions.head_title.list')) }}
            ...
            <table class="table {{config('theme.layout.table_list_item')}}">
                <thead class="{{config('theme.layout.table_list_item_head')}}">
                    <tr>
                        ...
                        <th class="text-center">
                            @sortablelink('slug',__('permissions.slug.th'))
                        </th>
                        @endif
                        @if($arrShowField['name']==true)
                        <th class="text-center">
                            @sortablelink('name',__('permissions.name.th'))
                        </th>
                        @endif
                        @if($arrShowField['description']==true)
                        <th class="text-center">
                            @sortablelink('description',__('permissions.description.th'))
                        </th>
                        @endif
                        @if($arrShowField['group_code']==true)
                        <th class="text-center">
                            @sortablelink('group_code',__('permissions.group_code.th'))
                        </th>
                        @endif
                        @if($arrShowField['active']==true)
                        <th class="text-center">
                            @sortablelink('active',__('permissions.active.th'))
                        </th>
                        @endif
                ...
                <tbody>
                    ...
                    <tr id="row_{{$item->id}}">
                        ...
                        @endif
                        @if(!empty( $arrShowField['group_code'] ))
                        <td>
                            {!! @$item->group_code !!}
                        </td>
                        @endif
                        ...
                        <td>
                            @include('components._badge_radio',['val'=>@$item->active,'tTrue'=>ucfirst(__('permissions.active.text_radio.true')), 'tFalse'=>ucfirst(__('permissions.active.text_radio.false'))])
                        </td>
                        @endif
...
<!-- END Page Content -->
@include('components._form_del',['action'=> 'permissions'])
