            ...
            <form action="{{ url('/api_users'.( isset($ApiUser->id) ? '/' . $ApiUser->id : '')) }}" method="POST" class="needs-validation" enctype="application/x-www-form-urlencoded" id="formUserApi" novalidate>
                ...
                <div class="row form-group">
                   ...
                   @if($arrShowField['description']==true)
                    ...
                    <div class="{{config('theme.layout.form')}}">
                        <label for="current-password">{{ucfirst(__('api_users.current_password.label'))}}
                            <span class="text-danger">*</span>
                            @if(__('api_users.current_password.popover.title') != "")
                            @include('components._popover_info', ['title' => ucfirst(__('api_users.current_password.popover.title')) ,'content'=> ucfirst(__('api_users.current_password.popover.content'))])
                            @endif
                        </label>
                        <input type="password" class="form-control" id="current_password" @if(isset($ApiUser->id)) required @endif autocomplete="false"  value="" placeholde="{{__('api_users.current_password.placeholder')}}">
                        @include('components._invalid_feedback',['required'=>'required','message'=>ucfirst(__('api_users.current_password.label')) ])
                    </div>

                    <div class="col-12 mt-4 mb-0">
                        <a href="#" id="openChangePass">{{ ucfirst(__('api_users.label.change_password')) }}</a>
                        <hr>
                    </div>
                    <div class="{{config('theme.layout.form')}} passHideEdit">
                        <label for="password">{{ucfirst(__('api_users.new_password.label'))}}
                            <span class="text-danger">*</span>
                            @if(__('api_users.new_password.popover.title') != "")
                            @include('components._popover_info', ['title' => ucfirst(__('api_users.new_password.popover.title')) ,'content'=> ucfirst(__('api_users.new_password.popover.content'))])
                            @endif
                        </label>
                        <input type="password" class="form-control pass_required" id="password" name="password"  autocomplete="false"  value="" placeholde="{{__('api_users.new_password.placeholder')}}">
                        @include('components._invalid_feedback',['required'=>'required','message'=>ucfirst(__('api_users.new_password.label')) ])
                    </div>
                    ...
                    <div class="{{config('theme.layout.form')}} passHideEdit">
                        ...
                        <input type="password" class="form-control pass_required" id="password_confirmation" autocomplete="false" value="" placeholde="{{__('users.password_confirmation.placeholder')}}">
...
<script>
        ...
        var validation = Array.prototype.filter.call(forms,async function(form) {
          form.addEventListener('submit',async function(event) {
            if (form.checkValidity() === false) {
                ...
                username = $('#username').val(username.toLowerCase()).val()

                if(currentPassword !== undefined){
                   
                    var res = await chkPassword(currentPassword);
                    if(res != true){
                        noitMessage('Error','error',"{{ucfirst(__('api_users.message_password_error'))}}");
                        return;
                    }else{
                        chkPassTrue = true;
                    }
               ...
               // var ChkUser = await checkPolicyUsername(username)
               ...
               if(password){
                ...
                var title = "{{ucfirst(__('api_users.message_confirm_update.title'))}}"
                var message = "{{ucfirst(__('api_users.message_confirm_update.message'))}}"

                 var confirm = await confirmMessage(title,message,'question');
              
                if(confirm == true){
                    if(chkPassTrue && !password){
    ...
    async function chkPassword(){
        var url = "{{ route('chk.passwordapi') }}";
        var username = $('#old_username').val();  
        var current_password = $('#current_password').val(); 

        res = $.post(url,{'username':username,'password':current_password})
        .then(function(response) {
            var decodedResponse = atob(response);
            var obj = JSON.parse(decodedResponse);       
            if(obj.code === 200){          
...
//--- edit
    ...
    $('.passHideEdit').hide();
    var isShowBlockPass = false ;

    $('#openChangePass').on( "click", function() {
        
        if ( isShowBlockPass === false ) {
            $('.passHideEdit').show();
            $('.pass_required').attr('required',true)
            isShowBlockPass = true
        }else{
            $('.passHideEdit').hide();
            $('.pass_required').attr('required',false)
            $('.pass_required').val('')
            isShowBlockPass = false
        }
    })
